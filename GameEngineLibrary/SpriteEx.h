...
struct AnimStateDefinition
{
	...
	bool canInterrupt = true;  //not being used currently but you can add this functionality yourself by modifying PlayState function
...
void RegisterOnAnimFrameCompleteCB(std::function<void()> cb, int frameNumberToCheck)
{
	onAnimFrameComplete = cb;
...
*/
	...
	AnimStateDefinition() {}
	AnimStateDefinition(short rm, short cm, short del, bool loop = false, bool interrupt = true)
